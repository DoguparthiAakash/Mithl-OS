...
#include "memory.h"
#include "process.h"
#include "semantic.h" // For sys_agent_op
...
#define SYS_AGENT_OP          200   // AIOS/Agent SDK Syscall
...
void syscall_handler(registers_t *regs) {
    ...
    // if (regs->eax == 200) serial_write("[SYSCALL] Agent Op\n");
    // print hex regs->eax
    ...
    switch (syscall_nr) {
        case SYS_WRITE:
            {
                ...
                for (uint32_t i = 0; i < count; i++) {
                    char c = buf[i];
                    console_putc(c);
                    serial_putc(c); // Add Serial Mirror
                }
                ret = count;
            }
            break;
        ...
        case SYS_WAITPID: // Linux: 7 (waitpid), 114 (wait4). Let's use custom or match Linux 32-bit: SYS_WAITPID=7
             // Wait... define at top
             {
                 int pid = regs->ebx;
                 int *status = (int*)regs->ecx;
                 int options = regs->edx;
                 extern int process_wait(int pid, int *status, int options);
                 ret = process_wait(pid, status, options);
             }
             break;
        ...
        case SYS_MITHL_LOG:
            {
                ...
                console_write(msg);
                serial_write(msg); // Add Serial Mirror
                ret = 0;
            }
            break;
        ...
        case SYS_AGENT_OP:
            {
                ...
                // regs->ecx = arg1
                // regs->edx = arg2
                ret = sys_agent_op(regs->ebx, (void*)regs->ecx, (void*)regs->edx);
            }
            break;
            break;
