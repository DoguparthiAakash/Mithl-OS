...
void gui_run(void)
{
    ...
    while ((event = list_pop_front(gui_mgr.event_queue)))
    {
        ...
        if (event->type == GUI_EVENT_KEY_PRESS) {
            ...
            // Feed Nuklear (Key)
            nk_mithl_handle_event(&nk_ctx, event);
    ...
    // Nuklear Input End & Logic
    nk_input_end(&nk_ctx);
    nk_mithl_demo(&nk_ctx);
    ...
    if (gui_mgr.needs_redraw) {
        ...
        // Render Nuklear on top
        if (gui_mgr.needs_redraw) nk_mithl_render(&nk_ctx);
