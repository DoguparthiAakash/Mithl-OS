...
// Load ELF from memory buffer (e.g. Multiboot Module)
uint32_t elf_load_from_memory(const char *name, void *addr, uint32_t size) {
    (void)size;
    console_log("[ELF] Loading from memory: ");
    console_log(name);
    console_log("\n");
    ...
    if (magic[0] != 0x7F || magic[1] != 'E' || magic[2] != 'L' || magic[3] != 'F') {
        console_write("[ELF] Error: Invalid ELF Magic.\n");
        return 0;
    }
    ...
    for (uint32_t i = 0; i < ph_count; i++) {
        Elf32_Phdr *ph = (Elf32_Phdr*)((uint8_t*)addr + ph_offset + (i * ph_size));
        ...
        if (ph->p_type == PT_LOAD) {
            ...
            for (uint32_t pg = start_page; pg < end_page; pg += 4096) {
                void *phys = pmm_alloc_block();
                if (!phys) return 0;
                vmm_map_page(phys, (void*)pg);
            }

            // Copy Data (Direct memcpy from memory buffer)
            if (ph->p_filesz > 0) {
                 memcpy((void*)ph->p_vaddr, (uint8_t*)addr + ph->p_offset, ph->p_filesz);
            }
    ...
    console_log("[ELF] Memory load success. Entry: ");
    // print hex
    console_log("\n");
    return hdr->e_entry;
}
