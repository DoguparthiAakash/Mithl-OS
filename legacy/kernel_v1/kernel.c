...
void launch_doom_safe(void) {
    if (doom_launched) {
        console_write("[KERNEL] Launching Doom skipped (Already launched)\n");
        return;
    }
    doom_launched = 1;
    console_write("[KERNEL] Launching Doom... (First Time)\n");
...
// Wrapper to execute init module
void launch_init_module(void) {
...
void kmain(uint32_t magic, void* addr)
{
    ...
    // static boot_info_t boot_info; // Use Global
    ...
    sched_init_polaris(); // Initialize Polaris Scheduler structures
    
    // Initialize Semantic Agent Core (AIOS Kernel)
    agent_init();
    
    // Launch Userspace Init (if present)
    launch_init_module();
