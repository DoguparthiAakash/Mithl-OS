...
void *memory_realloc(void *ptr, size_t size)
{
    ...
    // Safety check
    if (header->used == 0) return NULL; // Double free/Invalid ptr
    
    // For now, always allocate new and copy (Simple, Safe)
    // Optimization: Check if header->size >= size, then just return ptr (maybe split?)
    if (header->size >= size) {
        // Shrinking or same size. 
        // We could split block but lazy approach: do nothing.
        return ptr;
    }
    ...
    // Copy old data
    // We only copy what's valid in the OLD block
