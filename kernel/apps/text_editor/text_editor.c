...
int editor_move_cursor(text_editor_t *editor, int dx, int dy) {
    ...
    editor->cursor_x = editor->current_column;
    editor->cursor_y = editor->current_line;
...
int editor_get_content(text_editor_t *editor, char *buffer, int buffer_size)
{
    ...
    int offset = 0;
    for (int i = 0; i < editor->line_count && offset < buffer_size - 64; i++) {
        int line_len = strlen(editor->lines[i]);
...
static void editor_draw_content(gui_renderer_t *renderer, gui_element_t *element) {
    ...
    draw_circle_filled(gear_x, my + 4, 3, 0xFF606060);
    ...
    draw_text("1, Col 1", b.x + 30, status_y + 6, 0xFF606060, 0);
...
static void editor_handle_event(gui_element_t *element, gui_event_t *event) {
    ...
    if (event->type == GUI_EVENT_KEY_PRESS) {
         ...
         if (main_editor.mode == MODE_NORMAL) {
             ...
             else if (key == 'i') { main_editor.mode = MODE_INSERT; }
             else if (key == ':') {
             ...
             if (key == '\n' || key == 13) {
                 if (strcmp(main_editor.command_buffer, ":w") == 0) { if (editor_save_file(&main_editor) >= 0) {} }
                 else if (strcmp(main_editor.command_buffer, ":q") == 0) { main_editor.is_open = 0; }
                 else if (strcmp(main_editor.command_buffer, ":wq") == 0) { editor_save_file(&main_editor); main_editor.is_open = 0; }
                 main_editor.mode = MODE_NORMAL;
                 main_editor.cmd_len = 0;
                 gui_mgr.needs_redraw = 1;
             }
                 ...
                 if (main_editor.cmd_len > 0) {
                     ...
                     if (main_editor.cmd_len == 0) main_editor.mode = MODE_NORMAL;
                     gui_mgr.needs_redraw = 1;
                 }
             }
